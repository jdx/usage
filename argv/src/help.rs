...
fn page_sections(
    ...
    let before = if long {
        meta.before_long_help.or(meta.before_help)
    } else {
        meta.before_help
    };
    ...
    // Each row's usage once: it sizes the column, is the row's text, and is what colouring
    // looks for at the start of a row.
    ...
    examples_section(out, meta.examples, long);
    if let Some(known) = known.as_mut() {
    ...
    let after = if long {
        meta.after_long_help.or(meta.after_help)
    } else {
        meta.after_help
    };
