...
/// wrote by hand.
///
/// [`crate::install`] reads it back off disk to decide whether replacing a file is an upgrade or
/// a theft, so changing this string turns every already-installed script into a stranger's file.
/// `every_generated_script_carries_the_marker_the_installer_looks_for` is the guard.
pub(crate) const GENERATED_MARKER: &str = "@generated by usage-argv";
...
fn header(bin: &str, shell: Shell, comment: &str) -> String {
    format!(
        "{comment} {GENERATED_MARKER} for `{bin} __complete_word__ --shell {}`\n\
