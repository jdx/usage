...
When a spec defines a trailing variadic argument, usage sets the `usage__varargs_idx` environment
variable on the subprocess. Its value is the number of parsed args in `$@` that precede the
trailing varargs. A script can `shift` by this amount to isolate the varargs in `$@`:
