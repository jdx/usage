...
| `Option<T>`, `Vec<T>`, `Option<Vec<T>>`, `Option<Option<T>>` | yes        | yes  | yes   | yes | yes    | n/a        | Nested `Option` preserves absent, bare, and explicitly valued flags; values use `FromStr`.                                                              |
