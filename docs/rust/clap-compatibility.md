...
| ordered/deprecated environment aliases                       | usage-only | yes  | yes   | yes | yes    | no         | `env_fallback(…)` preserves declaration order; `deprecated_env(…)` is consulted last and remains visible in generated output.                           |
...
| `help_template`                                    | different | different | different | different | yes    | no      | Supported, with a closed vocabulary of six pre-rendered sections rather than clap's tags; see [Laying a page out](./help.md#laying-a-page-out) for the mapping. clap keeps `get_help_template` private, so the bridge cannot recover one. |
...
| built-in help/version action and flag control      | yes       | yes       | yes       | yes       | yes    | yes     | `Help`, `HelpShort`, `HelpLong`, and `Version` actions can relocate built-ins; usage additionally provides recursive `HelpAll`; each synthetic entry can be disabled.                                                                     |
