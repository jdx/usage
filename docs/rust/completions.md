...
file carrying usage's own `@generated` marker is `Wrote::Updated`; anything else is
`Error::Foreign` naming the path, unless the caller passes `OnForeign::Overwrite`. So re-running
an install after an upgrade needs no flag, while a script somebody wrote by hand survives one.
