...
var HelpText = argv.HelpTable{
	...
	{Key: FlagCd, ValueName: "DIR", Short: "Change directory before running command", Long: "Change directory before running command"},
	{Key: FlagEnv, Repeatable: true, ValueName: "ENV", Short: "Set the environment for loading `mise.<ENV>.toml`", Long: "Set the environment for loading `mise.<ENV>.toml`"},
	{Key: FlagForce, Hide: true, Short: "Force the operation", Long: "Force the operation"},
	{Key: FlagJobs, ValueName: "JOBS", Short: "How many jobs to run in parallel [default: 8]", Long: "How many jobs to run in parallel [default: 8]"},
	{Key: FlagDryRun, Hide: true, Short: "Dry run, don't actually do anything", Long: "Dry run, don't actually do anything"},
	{Key: FlagProfile, Hide: true, Repeatable: true, ValueName: "PROFILE", Short: "Set the profile (environment)", Long: "Set the profile (environment)"},
	{Key: FlagQuiet, Short: "Suppress non-error messages", Long: "Suppress non-error messages"},
	{Key: FlagShell, Hide: true, ValueName: "SHELL"},
	{Key: FlagTool, Hide: true, Repeatable: true, ValueName: "TOOL@VERSION", Short: "Tool(s) to run in addition to what is in mise.toml files e.g.: node@20 python@3.10", Long: "Tool(s) to run in addition to what is in mise.toml files e.g.: node@20 python@3.10"},
	...
	{Key: FlagLogLevel, Hide: true, ValueName: "LEVEL"},
	...
	{Key: FlagOutput, ValueName: "OUTPUT"},
	...
	{Key: FlagActivateShell, Hide: true, ValueName: "SHELL", Short: "Shell type to generate the script for", Long: "Shell type to generate the script for"},
	...
	{Key: ArgActivateShellType, Short: "Shell type to generate the script for", Long: "Shell type to generate the script for"},
	{Key: CmdToolAlias, Short: "Manage tool version aliases."},
	{Key: FlagToolAliasTool, ValueName: "TOOL", Short: "Filter aliases by tool", Long: "Filter aliases by tool"},
	...
	{Key: CmdBackends, Short: "Manage backends"},
	...
	{Key: ArgBinPathsToolVersion, Short: "Tool(s) to look up", Long: "Tool(s) to look up\ne.g.: ruby@3"},
	...
	{Key: FlagBootstrapOnly, Repeatable: true, ValueName: "ONLY", Short: "Run only one or more bootstrap parts", Long: "Run only one or more bootstrap parts\n\nCan be passed multiple times or as a comma-separated list. Cannot be used with `--skip`."},
	...
	{Key: FlagBootstrapSkip, Repeatable: true, ValueName: "SKIP", Short: "Skip one or more bootstrap parts", Long: "Skip one or more bootstrap parts\n\nCan be passed multiple times or as a comma-separated list."},
	...
	{Key: FlagBootstrapDotfilesAddMode, ValueName: "MODE", Short: "Dotfile mode to write", Long: "Dotfile mode to write"},
	...
	{Key: FlagBootstrapDotfilesAddPath, ValueName: "PATH", Short: "Write to this config file or directory", Long: "Write to this config file or directory"},
	{Key: FlagBootstrapDotfilesAddSource, ValueName: "PATH", Short: "Source path to use for a single target", Long: "Source path to use for a single target"},
	...
	{Key: CmdBootstrapDotfilesEdit, Short: "Edit a managed dotfile source"},
	...
	{Key: FlagBootstrapDotfilesEditMode, ValueName: "MODE", Short: "Dotfile mode to use if the target is not yet managed", Long: "Dotfile mode to use if the target is not yet managed"},
	{Key: FlagBootstrapDotfilesEditSource, ValueName: "PATH", Short: "Source path to use if the target is not yet managed", Long: "Source path to use if the target is not yet managed"},
	...
	{Key: CmdBootstrapDotfilesStatus, Short: "Show the status of dotfiles from `[dotfiles]`"},
	{Key: FlagBootstrapDotfilesStatusJson, Short: "Output in JSON format", Long: "Output in JSON format"},
	{Key: FlagBootstrapDotfilesStatusMissing, Short: "Exit with code 1 if any configured dotfiles are not in their desired", Long: "Exit with code 1 if any configured dotfiles are not in their desired\nstate (missing, source missing, differs)"},
	...
	{Key: FlagBootstrapPackagesApplyManager, ValueName: "MANAGER", Short: "Only install packages for this built-in or plugin manager", Long: "Only install packages for this built-in or plugin manager"},
	...
	{Key: CmdBootstrapPackagesBrewTap, Short: "Add a Homebrew tap URL to [bootstrap.brew.taps]"},
	...
	{Key: FlagBootstrapPackagesBrewTapPath, ValueName: "PATH", Short: "Write to this config file or directory", Long: "Write to this config file or directory"},
	...
	{Key: CmdBootstrapPackagesBrewUntap, Short: "Remove Homebrew tap URLs from [bootstrap.brew.taps]"},
	...
	{Key: FlagBootstrapPackagesBrewUntapPath, ValueName: "PATH", Short: "Write to this config file or directory", Long: "Write to this config file or directory"},
	...
	{Key: FlagBootstrapPackagesImportEnv, ValueName: "ENV", Short: "Write to the config file for this environment (mise.<ENV>.toml)", Long: "Write to the config file for this environment (mise.<ENV>.toml)"},
	...
	{Key: FlagBootstrapPackagesImportManager, ValueName: "MANAGER", Short: "Only import packages for this manager. Currently only `brew` is supported", Long: "Only import packages for this manager. Currently only `brew` is supported"},
	...
	{Key: FlagBootstrapPackagesImportPath, ValueName: "PATH", Short: "Write to this config file or directory", Long: "Write to this config file or directory"},
	...
	{Key: FlagBootstrapPackagesPruneManager, ValueName: "MANAGER", Short: "Only prune packages for this manager. Currently only `brew` is supported", Long: "Only prune packages for this manager. Currently only `brew` is supported"},
	...
	{Key: CmdBootstrapPackagesStatus, Short: "Show the status of system packages from `[bootstrap.packages]`"},
	...
	{Key: FlagBootstrapPackagesUpgradeManager, ValueName: "MANAGER", Short: "Only upgrade packages for this built-in or plugin manager", Long: "Only upgrade packages for this built-in or plugin manager"},
	...
	{Key: FlagBootstrapPackagesUseEnv, ValueName: "ENV", Short: "Write to the config file for this environment (mise.<ENV>.toml)", Long: "Write to the config file for this environment (mise.<ENV>.toml)"},
	...
	{Key: FlagBootstrapPackagesUsePath, ValueName: "PATH", Short: "Write to this config file or directory", Long: "Write to this config file or directory"},
	...
	{Key: FlagBootstrapRemoteBootstrapCommand, ValueName: "COMMAND", Short: "Explicit remote shell command that installs mise and places it on PATH", Long: "Explicit remote shell command that installs mise and places it on PATH"},
	{Key: FlagBootstrapRemoteConnectTimeout, ValueName: "CONNECT_TIMEOUT", Short: "SSH connection timeout in seconds", Long: "SSH connection timeout in seconds"},
	{Key: FlagBootstrapRemoteExclude, Repeatable: true, ValueName: "PATTERN", Short: "Additional archive pattern to exclude; repeat for multiple patterns", Long: "Additional archive pattern to exclude; repeat for multiple patterns"},
	...
	{Key: FlagBootstrapRemoteHost, Repeatable: true, ValueName: "[USER@]HOST", Short: "Ad-hoc SSH destination (`[user@]host`); repeat for multiple hosts", Long: "Ad-hoc SSH destination (`[user@]host`); repeat for multiple hosts"},
	{Key: FlagBootstrapRemoteIdentityFile, ValueName: "IDENTITY_FILE", Short: "SSH identity file override", Long: "SSH identity file override"},
	...
	{Key: FlagBootstrapRemoteMiseBin, ValueName: "MISE_BIN", Short: "Local mise binary to upload (escape hatch for custom architectures)", Long: "Local mise binary to upload (escape hatch for custom architectures)"},
	{Key: FlagBootstrapRemoteOnly, Repeatable: true, ValueName: "ONLY", Short: "Run only one or more remote bootstrap parts", Long: "Run only one or more remote bootstrap parts"},
	{Key: FlagBootstrapRemotePort, ValueName: "PORT", Short: "SSH port override", Long: "SSH port override"},
	...
	{Key: FlagBootstrapRemoteRemoteMise, ValueName: "COMMAND", Short: "Existing mise executable name or path; relative paths use the staged project", Long: "Existing mise executable name or path; relative paths use the staged project"},
	{Key: FlagBootstrapRemoteSkip, Repeatable: true, ValueName: "SKIP", Short: "Skip one or more remote bootstrap parts", Long: "Skip one or more remote bootstrap parts"},
	{Key: FlagBootstrapRemoteSource, ValueName: "SOURCE", Short: "Local directory archived and sent to each target", Long: "Local directory archived and sent to each target"},
	{Key: FlagBootstrapRemoteSshOption, Repeatable: true, ValueName: "OPTION", Short: "OpenSSH `-o` option; repeat for multiple options", Long: "OpenSSH `-o` option; repeat for multiple options"},
	{Key: FlagBootstrapRemoteTag, Repeatable: true, ValueName: "TAG", Short: "Select configured hosts with this tag; repeat to match any tag", Long: "Select configured hosts with this tag; repeat to match any tag"},
	...
	{Key: CmdBootstrapStatus, Short: "Show the aggregate bootstrap status"},
	...
	{Key: CmdCacheClear, Short: "Deletes all cache files in mise"},
	{Key: FlagCacheClearOutdate, Hide: true, Short: "Mark all cache files as old", Long: "Mark all cache files as old"},
	{Key: FlagCacheClearTask, ValueName: "TASK", Short: "Clear output cache entries for a task name or pattern", Long: "Clear output cache entries for a task name or pattern"},
	{Key: ArgCacheClearTool, Short: "Tool(s) to clear cache for e.g.: node, python", Long: "Tool(s) to clear cache for e.g.: node, python"},
	{Key: CmdCachePath, Short: "Show the cache directory path"},
	...
	{Key: FlagCompletionShell, Hide: true, ValueName: "SHELL_TYPE", Short: "Shell type to generate completions for", Long: "Shell type to generate completions for"},
	...
	{Key: CmdConfigGet, Short: "Display the value of a setting in a mise.toml file"},
	{Key: FlagConfigGetFile, ValueName: "FILE", Short: "The path to the mise.toml file to read", Long: "The path to the mise.toml file to read\n\nCan be a file path or directory. If a directory is provided, the config file in that directory is used.\n\nIf not provided, the nearest mise.toml file will be used"},
	{Key: ArgConfigGetKey, Short: "The path of the config to display", Long: "The path of the config to display"},
	{Key: CmdConfigLs, Short: "List config files currently in use"},
	...
	{Key: CmdConfigSet, Short: "Set the value of a setting in a mise.toml file"},
	{Key: FlagConfigSetFile, ValueName: "FILE", Short: "The path to the mise.toml file to edit", Long: "The path to the mise.toml file to edit\n\nCan be a file path or directory. If a directory is provided, the config file in that directory is used.\n\nIf not provided, the nearest mise.toml file will be used"},
	{Key: FlagConfigSetType, ValueName: "TYPE"},
	...
	{Key: FlagDotfilesAddMode, ValueName: "MODE", Short: "Dotfile mode to write", Long: "Dotfile mode to write"},
	...
	{Key: FlagDotfilesAddPath, ValueName: "PATH", Short: "Write to this config file or directory", Long: "Write to this config file or directory"},
	{Key: FlagDotfilesAddSource, ValueName: "PATH", Short: "Source path to use for a single target", Long: "Source path to use for a single target"},
	...
	{Key: CmdDotfilesEdit, Hide: true, Short: "Edit a managed dotfile source"},
	{Key: FlagDotfilesEditApply, Short: "Apply this target after the editor exits", Long: "Apply this target after the editor exits"},
	{Key: FlagDotfilesEditMode, ValueName: "MODE", Short: "Dotfile mode to use if the target is not yet managed", Long: "Dotfile mode to use if the target is not yet managed"},
	{Key: FlagDotfilesEditSource, ValueName: "PATH", Short: "Source path to use if the target is not yet managed", Long: "Source path to use if the target is not yet managed"},
	...
	{Key: CmdDotfilesStatus, Hide: true, Short: "Show the status of dotfiles from `[dotfiles]`"},
	{Key: FlagDotfilesStatusJson, Short: "Output in JSON format", Long: "Output in JSON format"},
	{Key: FlagDotfilesStatusMissing, Short: "Exit with code 1 if any configured dotfiles are not in their desired", Long: "Exit with code 1 if any configured dotfiles are not in their desired\nstate (missing, source missing, differs)"},
	...
	{Key: CmdDoctor, Short: "Check mise installation for possible problems"},
	{Key: FlagDoctorJson},
	{Key: CmdDoctorPath, Short: "Print the current PATH entries mise is providing"},
	...
	{Key: FlagEnShell, ValueName: "SHELL", Short: "Shell to start", Long: "Shell to start\n\nDefaults to $SHELL"},
	{Key: ArgEnDir, Short: "Directory to start the shell in", Long: "Directory to start the shell in"},
	...
	{Key: FlagEnvShell, ValueName: "SHELL", Short: "Shell type to generate environment variables for", Long: "Shell type to generate environment variables for"},
	...
	{Key: FlagExecCommand, ValueName: "C", Short: "Command string to execute", Long: "Command string to execute"},
	{Key: FlagExecJobs, ValueName: "JOBS", Short: "Number of jobs to run in parallel", Long: "Number of jobs to run in parallel\n[default: 4]"},
	{Key: FlagExecAllowEnv, Repeatable: true, ValueName: "VAR", Short: "Allow specific env var through (implies --deny-env for everything else)", Long: "Allow specific env var through (implies --deny-env for everything else)\nSupports wildcards, e.g. --allow-env='MYAPP_*'"},
	{Key: FlagExecAllowNet, Repeatable: true, ValueName: "HOST", Short: "Allow network to specific host (implies --deny-net for everything else)", Long: "Allow network to specific host (implies --deny-net for everything else)\nmacOS only in v1; on Linux falls back to allowing all network"},
	{Key: FlagExecAllowRead, Repeatable: true, ValueName: "PATH", Short: "Allow reads from specific path (implies --deny-read for everything else)", Long: "Allow reads from specific path (implies --deny-read for everything else)"},
	{Key: FlagExecAllowWrite, Repeatable: true, ValueName: "PATH", Short: "Allow writes to specific path (implies --deny-write for everything else)", Long: "Allow writes to specific path (implies --deny-write for everything else)"},
	...
	{Key: CmdGenerate, Short: "Generate files for various tools/services"},
	...
	{Key: FlagGenerateBootstrapVersion, ValueName: "VERSION", Short: "Specify mise version to fetch", Long: "Specify mise version to fetch"},
	...
	{Key: FlagGenerateBootstrapLocalizedDir, ValueName: "LOCALIZED_DIR", Short: "Directory to put localized data into", Long: "Directory to put localized data into"},
	{Key: CmdGenerateConfig, Short: "Generate a mise.toml file"},
	...
	{Key: FlagGenerateConfigToolVersions, ValueName: "TOOL_VERSIONS", Short: "Path to a .tool-versions file to import tools from", Long: "Path to a .tool-versions file to import tools from"},
	{Key: ArgGenerateConfigPath, Short: "Path to the config file to create", Long: "Path to the config file to create"},
	{Key: CmdGenerateDevcontainer, Short: "Generate a devcontainer to execute mise"},
	{Key: FlagGenerateDevcontainerImage, ValueName: "IMAGE", Short: "The image to use for the devcontainer", Long: "The image to use for the devcontainer"},
	...
	{Key: FlagGenerateDevcontainerName, ValueName: "NAME", Short: "The name of the devcontainer", Long: "The name of the devcontainer"},
	...
	{Key: FlagGenerateGitPreCommitTask, ValueName: "TASK", Short: "The task to run when the pre-commit hook is triggered", Long: "The task to run when the pre-commit hook is triggered"},
	...
	{Key: FlagGenerateGitPreCommitHook, ValueName: "HOOK", Short: "Which hook to generate (saves to .git/hooks/$hook)", Long: "Which hook to generate (saves to .git/hooks/$hook)"},
	...
	{Key: FlagGenerateGithubActionTask, ValueName: "TASK", Short: "The task to run when the workflow is triggered", Long: "The task to run when the workflow is triggered"},
	{Key: FlagGenerateGithubActionWrite, Short: "write to .github/workflows/$name.yml", Long: "write to .github/workflows/$name.yml"},
	{Key: FlagGenerateGithubActionName, ValueName: "NAME", Short: "the name of the workflow to generate", Long: "the name of the workflow to generate"},
	{Key: CmdGenerateTaskDocs, Short: "Generate documentation for tasks in a project"},
	...
	{Key: FlagGenerateTaskDocsOutput, ValueName: "OUTPUT", Short: "writes the generated docs to a file/directory", Long: "writes the generated docs to a file/directory"},
	{Key: FlagGenerateTaskDocsRoot, ValueName: "ROOT", Short: "root directory to search for tasks", Long: "root directory to search for tasks"},
	{Key: FlagGenerateTaskDocsStyle, ValueName: "STYLE"},
	...
	{Key: FlagGenerateTaskStubsDir, ValueName: "DIR", Short: "Directory to create task stubs inside of", Long: "Directory to create task stubs inside of"},
	{Key: FlagGenerateTaskStubsMiseBin, ValueName: "MISE_BIN", Short: "Path to a mise bin to use when running the task stub.", Long: "Path to a mise bin to use when running the task stub.\n\nUse `--mise-bin=./bin/mise` to use a mise bin generated from `mise generate bootstrap`"},
	...
	{Key: FlagGenerateToolStubBin, ValueName: "BIN", Short: "Binary path within the extracted archive", Long: "Binary path within the extracted archive\n\nIf not specified and the archive is downloaded, will auto-detect the most likely binary"},
	...
	{Key: FlagGenerateToolStubBootstrapVersion, ValueName: "BOOTSTRAP_VERSION", Short: "Specify mise version for the bootstrap script", Long: "Specify mise version for the bootstrap script\n\nBy default, uses the latest version from the install script.\nUse this to pin to a specific version (e.g., \"2025.1.0\")."},
	...
	{Key: FlagGenerateToolStubHttp, ValueName: "HTTP", Short: "HTTP backend type to use", Long: "HTTP backend type to use"},
	...
	{Key: FlagGenerateToolStubPlatformBin, Repeatable: true, ValueName: "PLATFORM_BIN", Short: "Platform-specific binary paths in the format platform:path", Long: "Platform-specific binary paths in the format platform:path\n\nExamples: --platform-bin windows-x64:tool.exe --platform-bin linux-x64:bin/tool"},
	{Key: FlagGenerateToolStubPlatformUrl, Repeatable: true, ValueName: "PLATFORM_URL", Short: "Platform-specific URLs in the format platform:url or just url (auto-detect platform)", Long: "Platform-specific URLs in the format platform:url or just url (auto-detect platform)\n\nWhen the output file already exists, new platforms will be appended to the existing platforms table. Existing platform URLs will be updated if specified again.\n\nIf only a URL is provided (without platform:), the platform will be automatically detected from the URL filename.\n\nExamples: --platform-url linux-x64:https://... --platform-url https://nodejs.org/dist/v22.17.1/node-v22.17.1-darwin-arm64.tar.gz"},
	...
	{Key: FlagGenerateToolStubUrl, ValueName: "URL", Short: "URL for downloading the tool", Long: "URL for downloading the tool\n\nExample: https://github.com/owner/repo/releases/download/v2.0.0/tool-linux-x64.tar.gz"},
	{Key: FlagGenerateToolStubVersion, ValueName: "VERSION", Short: "Version of the tool", Long: "Version of the tool"},
	...
	{Key: ArgGithubTokenHost, Short: "GitHub hostname", Long: "GitHub hostname"},
	...
	{Key: FlagGlobalFuzzy, Short: "Save fuzzy version to `~/.tool-versions`", Long: "Save fuzzy version to `~/.tool-versions`\ne.g.: `mise global --fuzzy node@20` will save `node 20` to ~/.tool-versions\nthis is the default behavior unless MISE_ASDF_COMPAT=1"},
	{Key: FlagGlobalPath, Short: "Get the path of the global config file", Long: "Get the path of the global config file"},
	{Key: FlagGlobalPin, Short: "Save exact version to `~/.tool-versions`", Long: "Save exact version to `~/.tool-versions`\ne.g.: `mise global --pin node@20` will save `node 20.0.0` to ~/.tool-versions"},
	{Key: FlagGlobalRemove, Repeatable: true, ValueName: "TOOL", Short: "Remove the tool(s) from ~/.tool-versions", Long: "Remove the tool(s) from ~/.tool-versions"},
	{Key: ArgGlobalToolVersion, Short: "Tool(s) to add to .tool-versions", Long: "Tool(s) to add to .tool-versions\ne.g.: node@20\nIf this is a single tool with no version, the current value of the global\n.tool-versions will be displayed"},
	...
	{Key: FlagHookEnvShell, ValueName: "SHELL", Short: "Shell type to generate script for", Long: "Shell type to generate script for"},
	{Key: FlagHookEnvReason, Hide: true, ValueName: "REASON", Short: "Reason for calling hook-env (e.g., \"precmd\", \"chpwd\")", Long: "Reason for calling hook-env (e.g., \"precmd\", \"chpwd\")"},
	...
	{Key: FlagHookNotFoundShell, ValueName: "SHELL", Short: "Shell type to generate script for", Long: "Shell type to generate script for"},
	...
	{Key: CmdEdit, Short: "Edit mise.toml interactively"},
	...
	{Key: FlagEditToolVersions, ValueName: "TOOL_VERSIONS", Short: "Path to a .tool-versions file to import tools from", Long: "Path to a .tool-versions file to import tools from"},
	...
	{Key: FlagInstallJobs, ValueName: "JOBS", Short: "Number of jobs to run in parallel", Long: "Number of jobs to run in parallel\n[default: 4]"},
	...
	{Key: FlagInstallMinimumReleaseAge, ValueName: "MINIMUM_RELEASE_AGE", Short: "Only install versions released before this date or older than this duration", Long: "Only install versions released before this date or older than this duration\n\nSupports absolute dates like \"2024-06-01\" and relative durations like \"90d\" or \"1y\"."},
	...
	{Key: FlagLatestMinimumReleaseAge, ValueName: "MINIMUM_RELEASE_AGE", Short: "Only consider versions released before this date or older than this duration", Long: "Only consider versions released before this date or older than this duration\n\nSupports absolute dates like \"2024-06-01\" and relative durations like \"90d\" or \"1y\".\nOverrides per-tool `minimum_release_age` options and the global `minimum_release_age` setting."},
	...
	{Key: ArgLinkPath, Demanded: true, Short: "The local path to the tool version", Long: "The local path to the tool version\ne.g.: ~/.nvm/versions/node/v20.0.0"},
	...
	{Key: FlagLocalParent, Short: "Recurse up to find a .tool-versions file rather than using the current directory only", Long: "Recurse up to find a .tool-versions file rather than using the current directory only\nby default this command will only set the tool in the current directory (\"$PWD/.tool-versions\")"},
	...
	{Key: FlagLocalPin, Short: "Save exact version to `.tool-versions`", Long: "Save exact version to `.tool-versions`\ne.g.: `mise local --pin node@20` will save `node 20.0.0` to .tool-versions"},
	{Key: FlagLocalRemove, Repeatable: true, ValueName: "TOOL", Short: "Remove the tool(s) from .tool-versions", Long: "Remove the tool(s) from .tool-versions"},
	{Key: ArgLocalToolVersion, Short: "Tool(s) to add to .tool-versions/mise.toml", Long: "Tool(s) to add to .tool-versions/mise.toml\ne.g.: node@20\nif this is a single tool with no version,\nthe current value of .tool-versions/mise.toml will be displayed"},
	...
	{Key: FlagLockGlobal, Short: "Target only global config lockfiles (~/.config/mise/mise.lock and system config)", Long: "Target only global config lockfiles (~/.config/mise/mise.lock and system config)\nBy default, only the active project config root is locked"},
	{Key: FlagLockJobs, ValueName: "JOBS", Short: "Number of jobs to run in parallel", Long: "Number of jobs to run in parallel"},
	...
	{Key: FlagLockPlatform, Repeatable: true, ValueName: "PLATFORM", Short: "Comma-separated list of platforms to target", Long: "Comma-separated list of platforms to target\ne.g.: linux-x64,macos-arm64,windows-x64\nIf not specified, all platforms already in lockfile will be updated"},
	...
	{Key: FlagLockLocal, Short: "Update mise.local.lock instead of mise.lock", Long: "Update mise.local.lock instead of mise.lock\nUse for tools defined in .local.toml configs"},
	{Key: FlagLockMinimumReleaseAge, ValueName: "MINIMUM_RELEASE_AGE", Short: "Only lock versions released before this age or date", Long: "Only lock versions released before this age or date\n\nSupports absolute dates like \"2024-06-01\" and relative durations like \"90d\" or \"1y\".\nThis only affects fuzzy version matches like \"20\" or \"latest\".\nExplicitly pinned versions like \"22.5.0\" are not filtered.\nExisting matching lockfile entries are preserved and are not downgraded solely by this flag."},
	{Key: ArgLockTool, Short: "Tool(s) to update in lockfile", Long: "Tool(s) to update in lockfile\ne.g.: node python\nIf not specified, all tools in lockfile will be updated"},
	...
	{Key: FlagLsPlugin, Hide: true, ValueName: "TOOL_FLAG"},
	...
	{Key: FlagLsPrefix, ValueName: "PREFIX", Short: "Display versions matching this prefix", Long: "Display versions matching this prefix"},
	...
	{Key: FlagLsRemoteMinimumReleaseAge, ValueName: "MINIMUM_RELEASE_AGE", Short: "Only show versions released before this age or date", Long: "Only show versions released before this age or date\n\nSupports absolute dates like \"2024-06-01\" and relative durations like \"90d\" or \"1y\"."},
	...
	{Key: FlagLsRemotePrerelease, Short: "Include pre-release versions in the output for backends that report", Long: "Include pre-release versions in the output for backends that report\nupstream prerelease metadata or opt in to regex-based prerelease\ndetection. Equivalent to setting `MISE_PRERELEASES=1` or the\n`prereleases` setting for the duration of this command."},
	...
	{Key: ArgLsRemotePrefix, Short: "The version prefix to use when querying the latest version", Long: "The version prefix to use when querying the latest version\nsame as the first argument after the \"@\""},
	...
	{Key: FlagOciBuildCopy, Repeatable: true, ValueName: "HOST_PATH:IMAGE_PATH", Short: "Copy a host file, directory, or symlink into the image (repeatable, HOST:IMAGE)", Long: "Copy a host file, directory, or symlink into the image (repeatable, HOST:IMAGE)"},
	{Key: FlagOciBuildOutput, ValueName: "OUTPUT", Short: "Output directory for the OCI image layout", Long: "Output directory for the OCI image layout"},
	{Key: FlagOciBuildFrom, ValueName: "FROM", Short: "Base image reference (overrides [oci].from and the oci.default_from setting)", Long: "Base image reference (overrides [oci].from and the oci.default_from setting)"},
	...
	{Key: FlagOciBuildTag, ValueName: "TAG", Short: "Tag to record in the image index (the org.opencontainers.image.ref.name annotation)", Long: "Tag to record in the image index (the org.opencontainers.image.ref.name annotation)"},
	{Key: FlagOciBuildMountPoint, ValueName: "MOUNT_POINT", Short: "Where to place tool installs inside the image (default: /mise)", Long: "Where to place tool installs inside the image (default: /mise)"},
	...
	{Key: FlagOciBuildOwner, ValueName: "UID[:GID]", Short: "UID[:GID] to assign to every tar entry in generated layers", Long: "UID[:GID] to assign to every tar entry in generated layers\n\nOverrides [oci].user_id / [oci].group_id. Defaults to 0:0. If GID is omitted, it defaults to UID. This affects file ownership only; [oci].user controls the image USER directive."},
	...
	{Key: FlagOciPushCacheFrom, ValueName: "REF", Short: "Reuse unchanged tool layers from this image instead of the destination ref", Long: "Reuse unchanged tool layers from this image instead of the destination ref\n\nMust live in the same repository as the destination. Useful when each push gets a unique tag (e.g. per-commit tags in CI): `--cache-from ghcr.io/me/dev:latest ghcr.io/me/dev:$SHA`."},
	{Key: FlagOciPushFrom, ValueName: "FROM", Short: "Base image for the build (ignored with --image-dir)", Long: "Base image for the build (ignored with --image-dir)"},
	{Key: FlagOciPushImageDir, ValueName: "IMAGE_DIR", Short: "Push an already-built OCI image layout (skip the build step)", Long: "Push an already-built OCI image layout (skip the build step)"},
	...
	{Key: FlagOciPushMountPoint, ValueName: "MOUNT_POINT", Short: "Override in-image mount point (ignored with --image-dir)", Long: "Override in-image mount point (ignored with --image-dir)"},
	...
	{Key: FlagOciPushOwner, ValueName: "UID[:GID]", Short: "UID[:GID] to assign to every tar entry when building (conflicts with --image-dir)", Long: "UID[:GID] to assign to every tar entry when building (conflicts with --image-dir)\n\nOverrides [oci].user_id / [oci].group_id. Defaults to 0:0. If GID is omitted, it defaults to UID. This affects file ownership only; [oci].user controls the image USER directive."},
	...
	{Key: FlagOciRunEngine, ValueName: "ENGINE", Short: "Container engine to use (`auto`, `podman`, or `docker`)", Long: "Container engine to use (`auto`, `podman`, or `docker`)"},
	{Key: FlagOciRunFrom, ValueName: "FROM", Short: "Base image reference for the build (ignored with --image-dir)", Long: "Base image reference for the build (ignored with --image-dir)"},
	{Key: FlagOciRunImageDir, ValueName: "IMAGE_DIR", Short: "Use an already-built OCI image layout instead of building fresh", Long: "Use an already-built OCI image layout instead of building fresh"},
	...
	{Key: FlagOciRunMountPoint, ValueName: "MOUNT_POINT", Short: "Override in-image mount point (ignored with --image-dir)", Long: "Override in-image mount point (ignored with --image-dir)"},
	...
	{Key: FlagOciRunOwner, ValueName: "UID[:GID]", Short: "UID[:GID] to assign to every tar entry when building (conflicts with --image-dir)", Long: "UID[:GID] to assign to every tar entry when building (conflicts with --image-dir)\n\nOverrides [oci].user_id / [oci].group_id. Defaults to 0:0. If GID is omitted, it defaults to UID. This affects file ownership only; [oci].user controls the image USER directive."},
	{Key: FlagOciRunVolume, Repeatable: true, ValueName: "HOST:CONTAINER", Short: "Bind-mount a host path (repeatable, `HOST:CONTAINER[:MODE]`)", Long: "Bind-mount a host path (repeatable, `HOST:CONTAINER[:MODE]`)\n\nNote: unlike `docker run -v`, there's no `-v` short flag here because mise reserves `-v` for --verbose. Use `--volume` or `--mount`."},
	{Key: FlagOciRunEnv, Repeatable: true, ValueName: "KEY=VAL", Short: "Set environment variable in the container (repeatable, `KEY=VAL`)", Long: "Set environment variable in the container (repeatable, `KEY=VAL`)"},
	...
	{Key: FlagOciRunWorkdir, ValueName: "WORKDIR", Short: "Working directory inside the container", Long: "Working directory inside the container"},
	...
	{Key: ArgOutdatedToolVersion, Short: "Tool(s) to show outdated versions for", Long: "Tool(s) to show outdated versions for\ne.g.: node@20 python@3.10\nIf not specified, all tools in global and local configs will be shown"},
	...
	{Key: CmdPlugins, Short: "Manage plugins"},
	...
	{Key: FlagPluginsCore, Short: "The built-in plugins only", Long: "The built-in plugins only\nNormally these are not shown"},
	{Key: FlagPluginsUrls, Short: "Show the git url for each plugin", Long: "Show the git url for each plugin\ne.g.: https://github.com/mise-plugins/vfox-cmake.git"},
	{Key: FlagPluginsRefs, Hide: true, Short: "Show the git refs for each plugin", Long: "Show the git refs for each plugin\ne.g.: main 1234abc"},
	...
	{Key: FlagPluginsInstallAll, Short: "Install all missing plugins", Long: "Install all missing plugins\nThis will only install plugins that have matching shorthands.\ni.e.: they don't need the full git repo url"},
	{Key: FlagPluginsInstallForce, Short: "Reinstall even if plugin exists", Long: "Reinstall even if plugin exists"},
	{Key: FlagPluginsInstallJobs, ValueName: "JOBS", Short: "Number of jobs to run in parallel", Long: "Number of jobs to run in parallel"},
	{Key: FlagPluginsInstallVerbose, Repeatable: true, Short: "Show installation output", Long: "Show installation output"},
	{Key: ArgPluginsInstallNewPlugin, Short: "The name of the plugin to install", Long: "The name of the plugin to install\ne.g.: cmake, poetry\nCan specify multiple plugins: `mise plugins install cmake poetry`"},
	...
	{Key: ArgPluginsLinkName, Demanded: true, Short: "The name of the plugin", Long: "The name of the plugin\ne.g.: cmake, poetry"},
	{Key: ArgPluginsLinkDir, Short: "The local path to the plugin", Long: "The local path to the plugin\ne.g.: ./vfox-cmake"},
	{Key: CmdPluginsLs, Short: "List installed plugins", Long: "List installed plugins\n\nCan also show remotely available plugins to install."},
	{Key: FlagPluginsLsAll, Hide: true, Short: "List all available remote plugins", Long: "List all available remote plugins\nSame as `mise plugins ls-remote`"},
	{Key: FlagPluginsLsCore, Hide: true, Short: "The built-in plugins only", Long: "The built-in plugins only\nNormally these are not shown"},
	{Key: FlagPluginsLsOutdated, Short: "Show plugins with available updates", Long: "Show plugins with available updates\nChecks the remote for newer versions and only displays plugins that are outdated"},
	{Key: FlagPluginsLsUrls, Short: "Show the git url for each plugin", Long: "Show the git url for each plugin\ne.g.: https://github.com/mise-plugins/vfox-cmake.git"},
	{Key: FlagPluginsLsRefs, Hide: true, Short: "Show the git refs for each plugin", Long: "Show the git refs for each plugin\ne.g.: main 1234abc"},
	...
	{Key: CmdPluginsUninstall, Short: "Removes a plugin"},
	...
	{Key: FlagPluginsUpdateJobs, ValueName: "JOBS", Short: "Number of jobs to run in parallel", Long: "Number of jobs to run in parallel\nDefault: 4"},
	...
	{Key: FlagDepsOnly, Repeatable: true, ValueName: "ONLY", Short: "Run specific deps rule(s) only", Long: "Run specific deps rule(s) only"},
	{Key: FlagDepsSkip, Repeatable: true, ValueName: "SKIP", Short: "Skip specific deps rule(s)", Long: "Skip specific deps rule(s)"},
	...
	{Key: FlagDepsInstallOnly, Repeatable: true, ValueName: "ONLY", Short: "Run specific deps rule(s) only", Long: "Run specific deps rule(s) only"},
	{Key: FlagDepsInstallSkip, Repeatable: true, ValueName: "SKIP", Short: "Skip specific deps rule(s)", Long: "Skip specific deps rule(s)"},
	...
	{Key: FlagRegistryBackend, ValueName: "BACKEND", Short: "Show only tools for this backend", Long: "Show only tools for this backend"},
	...
	{Key: FlagRunAffectedBase, ValueName: "REV", Short: "Git base revision for --affected", Long: "Git base revision for --affected\nDefaults to MISE_AFFECTED_BASE, CI metadata, or HEAD~1"},
	...
	{Key: FlagRunAffectedHead, ValueName: "REV", Short: "Git head revision for --affected", Long: "Git head revision for --affected\nDefaults to MISE_AFFECTED_HEAD, CI metadata, or HEAD"},
	...
	{Key: FlagRunCd, ValueName: "CD", Short: "Change to this directory before executing the command", Long: "Change to this directory before executing the command"},
	...
	{Key: FlagRunJobs, ValueName: "JOBS", Short: "Number of tasks to run in parallel", Long: "Number of tasks to run in parallel\n[default: 4]\nConfigure with `jobs` config or `MISE_JOBS` env var"},
	...
	{Key: FlagRunOutput, ValueName: "OUTPUT", Short: "Change how tasks information is output when running tasks", Long: "Change how tasks information is output when running tasks\n\n- `prefix` - Print stdout/stderr by line, prefixed with the task's label\n- `interleave` - Print directly to stdout/stderr instead of by line\n- `replacing` - Stdout is replaced each time, stderr is printed as is\n- `timed` - Only show stdout lines if they are displayed for more than 1 second\n- `keep-order` - Print stdout/stderr by line, prefixed with the task's label, but keep the order of the output\n- `quiet` - Don't show extra output\n- `silent` - Don't show any output including stdout and stderr from the task except for errors"},
	{Key: FlagRunQuiet, Short: "Don't show extra output", Long: "Don't show extra output"},
	{Key: FlagRunRaw, Short: "Read/write directly to stdin/stdout/stderr instead of by line", Long: "Read/write directly to stdin/stdout/stderr instead of by line\nRedactions are not applied with this option\nConfigure with `raw` config or `MISE_RAW` env var"},
	{Key: FlagRunShell, ValueName: "SHELL", Short: "Shell to use to run toml tasks", Long: "Shell to use to run toml tasks\n\nDefaults to `sh -c -o errexit -o pipefail` on unix, and `cmd /c` on Windows\nCan also be set with the setting `MISE_UNIX_DEFAULT_INLINE_SHELL_ARGS` or `MISE_WINDOWS_DEFAULT_INLINE_SHELL_ARGS`\nOr it can be overridden with the `shell` property on a task."},
	{Key: FlagRunSilent, Short: "Don't show any output except for errors", Long: "Don't show any output except for errors"},
	{Key: FlagRunTool, Repeatable: true, ValueName: "TOOL@VERSION", Short: "Tool(s) to run in addition to what is in mise.toml files e.g.: node@20 python@3.10", Long: "Tool(s) to run in addition to what is in mise.toml files e.g.: node@20 python@3.10"},
	{Key: FlagRunAllowEnv, Repeatable: true, ValueName: "VAR", Short: "Allow specific env var through (implies --deny-env for everything else)", Long: "Allow specific env var through (implies --deny-env for everything else)\nSupports wildcards, e.g. --allow-env='MYAPP_*'"},
	{Key: FlagRunAllowNet, Repeatable: true, ValueName: "HOST", Short: "Allow network to specific host (implies --deny-net for everything else)", Long: "Allow network to specific host (implies --deny-net for everything else)"},
	{Key: FlagRunAllowRead, Repeatable: true, ValueName: "PATH", Short: "Allow reads from specific path (implies --deny-read for everything else)", Long: "Allow reads from specific path (implies --deny-read for everything else)"},
	{Key: FlagRunAllowWrite, Repeatable: true, ValueName: "PATH", Short: "Allow writes to specific path (implies --deny-write for everything else)", Long: "Allow writes to specific path (implies --deny-write for everything else)"},
	...
	{Key: FlagRunTimeout, ValueName: "TIMEOUT", Short: "Timeout for the task to complete", Long: "Timeout for the task to complete\ne.g.: 30s, 5m"},
	...
	{Key: FlagSearchMatchType, ValueName: "MATCH_TYPE", Short: "Match type: equal, contains, or fuzzy", Long: "Match type: equal, contains, or fuzzy"},
	...
	{Key: FlagSetEnv, ValueName: "ENV", Short: "Create/modify an environment-specific config file like .mise.<env>.toml", Long: "Create/modify an environment-specific config file like .mise.<env>.toml"},
	...
	{Key: FlagSetAgeKeyFile, ValueName: "PATH", Short: "[experimental] Age identity file for encryption", Long: "[experimental] Age identity file for encryption\n\nDefaults to ~/.config/mise/age.txt if it exists"},
	{Key: FlagSetAgeRecipient, Repeatable: true, ValueName: "RECIPIENT", Short: "[experimental] Age recipient (x25519 public key) for encryption", Long: "[experimental] Age recipient (x25519 public key) for encryption\n\nCan be used multiple times. Requires --age-encrypt."},
	{Key: FlagSetAgeSshRecipient, Repeatable: true, ValueName: "PATH_OR_PUBKEY", Short: "[experimental] SSH recipient (public key or path) for age encryption", Long: "[experimental] SSH recipient (public key or path) for age encryption\n\nCan be used multiple times. Requires --age-encrypt."},
	{Key: FlagSetComplete, Hide: true, Short: "Render completions", Long: "Render completions"},
	{Key: FlagSetFile, ValueName: "FILE", Short: "The TOML file to update", Long: "The TOML file to update\n\nCan be a file path or directory. If a directory is provided, will create/use mise.toml in that directory.\nDefaults to [`MISE_DEFAULT_CONFIG_FILENAME`](https://mise.jdx.dev/configuration.html#mise_default_config_filename) environment variable, or `mise.toml`.\nUse [`MISE_GLOBAL_CONFIG_FILE`](https://mise.jdx.dev/configuration.html#mise_global_config_file) to choose a different global config path."},
	...
	{Key: FlagSetRemove, Hide: true, Repeatable: true, ValueName: "ENV_KEY", Short: "Remove the environment variable from config file", Long: "Remove the environment variable from config file\n\nCan be used multiple times."},
	...
	{Key: ArgSetEnvVar, Short: "Environment variable(s) to set", Long: "Environment variable(s) to set\ne.g.: NODE_ENV=production"},
	...
	{Key: FlagShellJobs, ValueName: "JOBS", Short: "Number of jobs to run in parallel", Long: "Number of jobs to run in parallel\n[default: 4]"},
	...
	{Key: CmdShellAliasGet, Short: "Show the command for a shell alias"},
	...
	{Key: CmdSyncRuby, Short: "Symlinks all ruby tool versions from an external tool into mise"},
	{Key: FlagSyncRubyBrew, Short: "Get tool versions from Homebrew", Long: "Get tool versions from Homebrew"},
	{Key: CmdTasks, Short: "Manage tasks"},
	...
	{Key: FlagTasksAll, Short: "Load all tasks from the entire monorepo, including sibling directories.", Long: "Load all tasks from the entire monorepo, including sibling directories.\nBy default, only tasks from the current directory hierarchy are loaded."},
	...
	{Key: FlagTasksSort, ValueName: "COLUMN", Short: "Sort by column. Default is name.", Long: "Sort by column. Default is name."},
	{Key: FlagTasksSortOrder, ValueName: "SORT_ORDER", Short: "Sort order. Default is asc.", Long: "Sort order. Default is asc."},
	...
	{Key: FlagTasksAddAlias, Repeatable: true, ValueName: "ALIAS", Short: "Other names for the task", Long: "Other names for the task"},
	{Key: FlagTasksAddDepends, Repeatable: true, ValueName: "DEPENDS", Short: "Add dependencies to the task", Long: "Add dependencies to the task"},
	{Key: FlagTasksAddDir, ValueName: "DIR", Short: "Run the task in a specific directory", Long: "Run the task in a specific directory"},
	...
	{Key: FlagTasksAddSources, Repeatable: true, ValueName: "SOURCES", Short: "Glob patterns of files this task uses as input", Long: "Glob patterns of files this task uses as input"},
	{Key: FlagTasksAddWaitFor, Repeatable: true, ValueName: "WAIT_FOR", Short: "Wait for these tasks to complete if they are to run", Long: "Wait for these tasks to complete if they are to run"},
	{Key: FlagTasksAddDependsPost, Repeatable: true, ValueName: "DEPENDS_POST", Short: "Dependencies to run after the task runs", Long: "Dependencies to run after the task runs"},
	{Key: FlagTasksAddDescription, ValueName: "DESCRIPTION", Short: "Description of the task", Long: "Description of the task"},
	{Key: FlagTasksAddOutputs, Repeatable: true, ValueName: "OUTPUTS", Short: "Glob patterns of files this task creates, to skip if they are not modified", Long: "Glob patterns of files this task creates, to skip if they are not modified"},
	{Key: FlagTasksAddRunWindows, ValueName: "RUN_WINDOWS", Short: "Command to run on windows", Long: "Command to run on windows"},
	{Key: FlagTasksAddShell, ValueName: "SHELL", Short: "Run the task in a specific shell", Long: "Run the task in a specific shell"},
	...
	{Key: ArgTasksAddRun},
	{Key: CmdTasksDeps, Short: "Display a tree visualization of a dependency graph"},
	...
	{Key: ArgTasksDepsTasks, Short: "Tasks to show dependencies for", Long: "Tasks to show dependencies for\nCan specify multiple tasks by separating with spaces\ne.g.: mise tasks deps lint test check"},
	...
	{Key: CmdTasksGraph, Short: "[experimental] Inspect the workspace project graph"},
	...
	{Key: CmdTasksInfo, Short: "Get information about a task"},
	...
	{Key: FlagTasksLsAll, Short: "Load all tasks from the entire monorepo, including sibling directories.", Long: "Load all tasks from the entire monorepo, including sibling directories.\nBy default, only tasks from the current directory hierarchy are loaded."},
	...
	{Key: FlagTasksLsSort, ValueName: "COLUMN", Short: "Sort by column. Default is name.", Long: "Sort by column. Default is name."},
	{Key: FlagTasksLsSortOrder, ValueName: "SORT_ORDER", Short: "Sort order. Default is asc.", Long: "Sort order. Default is asc."},
	...
	{Key: FlagTasksRunAffectedBase, ValueName: "REV", Short: "Git base revision for --affected", Long: "Git base revision for --affected\nDefaults to MISE_AFFECTED_BASE, CI metadata, or HEAD~1"},
	...
	{Key: FlagTasksRunAffectedHead, ValueName: "REV", Short: "Git head revision for --affected", Long: "Git head revision for --affected\nDefaults to MISE_AFFECTED_HEAD, CI metadata, or HEAD"},
	...
	{Key: FlagTasksRunCd, ValueName: "CD", Short: "Change to this directory before executing the command", Long: "Change to this directory before executing the command"},
	...
	{Key: FlagTasksRunJobs, ValueName: "JOBS", Short: "Number of tasks to run in parallel", Long: "Number of tasks to run in parallel\n[default: 4]\nConfigure with `jobs` config or `MISE_JOBS` env var"},
	...
	{Key: FlagTasksRunOutput, ValueName: "OUTPUT", Short: "Change how tasks information is output when running tasks", Long: "Change how tasks information is output when running tasks\n\n- `prefix` - Print stdout/stderr by line, prefixed with the task's label\n- `interleave` - Print directly to stdout/stderr instead of by line\n- `replacing` - Stdout is replaced each time, stderr is printed as is\n- `timed` - Only show stdout lines if they are displayed for more than 1 second\n- `keep-order` - Print stdout/stderr by line, prefixed with the task's label, but keep the order of the output\n- `quiet` - Don't show extra output\n- `silent` - Don't show any output including stdout and stderr from the task except for errors"},
	{Key: FlagTasksRunQuiet, Short: "Don't show extra output", Long: "Don't show extra output"},
	{Key: FlagTasksRunRaw, Short: "Read/write directly to stdin/stdout/stderr instead of by line", Long: "Read/write directly to stdin/stdout/stderr instead of by line\nRedactions are not applied with this option\nConfigure with `raw` config or `MISE_RAW` env var"},
	{Key: FlagTasksRunShell, ValueName: "SHELL", Short: "Shell to use to run toml tasks", Long: "Shell to use to run toml tasks\n\nDefaults to `sh -c -o errexit -o pipefail` on unix, and `cmd /c` on Windows\nCan also be set with the setting `MISE_UNIX_DEFAULT_INLINE_SHELL_ARGS` or `MISE_WINDOWS_DEFAULT_INLINE_SHELL_ARGS`\nOr it can be overridden with the `shell` property on a task."},
	{Key: FlagTasksRunSilent, Short: "Don't show any output except for errors", Long: "Don't show any output except for errors"},
	{Key: FlagTasksRunTool, Repeatable: true, ValueName: "TOOL@VERSION", Short: "Tool(s) to run in addition to what is in mise.toml files e.g.: node@20 python@3.10", Long: "Tool(s) to run in addition to what is in mise.toml files e.g.: node@20 python@3.10"},
	{Key: FlagTasksRunAllowEnv, Repeatable: true, ValueName: "VAR", Short: "Allow specific env var through (implies --deny-env for everything else)", Long: "Allow specific env var through (implies --deny-env for everything else)\nSupports wildcards, e.g. --allow-env='MYAPP_*'"},
	{Key: FlagTasksRunAllowNet, Repeatable: true, ValueName: "HOST", Short: "Allow network to specific host (implies --deny-net for everything else)", Long: "Allow network to specific host (implies --deny-net for everything else)"},
	{Key: FlagTasksRunAllowRead, Repeatable: true, ValueName: "PATH", Short: "Allow reads from specific path (implies --deny-read for everything else)", Long: "Allow reads from specific path (implies --deny-read for everything else)"},
	{Key: FlagTasksRunAllowWrite, Repeatable: true, ValueName: "PATH", Short: "Allow writes to specific path (implies --deny-write for everything else)", Long: "Allow writes to specific path (implies --deny-write for everything else)"},
	...
	{Key: FlagTasksRunTaskCache, ValueName: "TASK_CACHE", Short: "Set task output cache access for this run", Long: "Set task output cache access for this run\n\n- `read-write` - Read cached results and write new results\n- `read-only` - Read cached results without writing new results\n- `write-only` - Write new results without reading cached results\n- `off` - Disable task output caching\n- `local-only` - Read and write only the local cache; currently equivalent to `read-write`"},
	...
	{Key: FlagTasksRunTimeout, ValueName: "TIMEOUT", Short: "Timeout for the task to complete", Long: "Timeout for the task to complete\ne.g.: 30s, 5m"},
	...
	{Key: ArgTasksRunTask, Short: "Tasks to run", Long: "Tasks to run\nCan specify multiple tasks by separating with `:::`\ne.g.: mise run task1 arg1 arg2 ::: task2 arg1 arg2"},
	...
	{Key: CmdTasksValidate, Short: "Validate tasks for common errors and issues"},
	...
	{Key: ArgTasksValidateTasks, Short: "Tasks to validate", Long: "Tasks to validate\nIf not specified, validates all tasks"},
	{Key: CmdTestTool, Short: "Test a tool installs and executes"},
	{Key: FlagTestToolAll, Short: "Test every tool specified in registry/", Long: "Test every tool specified in registry/"},
	{Key: FlagTestToolJobs, ValueName: "JOBS", Short: "Number of tool tests to run in parallel", Long: "Number of tool tests to run in parallel\n[default: 4]"},
	...
	{Key: CmdTokenForgejo, Short: "Forgejo token"},
	{Key: FlagTokenForgejoUnmask, Short: "Show the full unmasked token", Long: "Show the full unmasked token"},
	{Key: ArgTokenForgejoHost, Short: "Forgejo hostname", Long: "Forgejo hostname"},
	{Key: CmdTokenGithub, Short: "GitHub token"},
	...
	{Key: ArgTokenGithubHost, Short: "GitHub hostname", Long: "GitHub hostname"},
	{Key: CmdTokenGitlab, Short: "GitLab token"},
	{Key: FlagTokenGitlabUnmask, Short: "Show the full unmasked token", Long: "Show the full unmasked token"},
	{Key: ArgTokenGitlabHost, Short: "GitLab hostname", Long: "GitLab hostname"},
	{Key: CmdTool, Short: "Gets information about a tool"},
	...
	{Key: FlagTrustShow, Short: "Show the trusted status of config files from the current directory and its parents.", Long: "Show the trusted status of config files from the current directory and its parents.\nDoes not trust or untrust any files."},
	...
	{Key: FlagUnuseEnv, ValueName: "ENV", Short: "Create/modify an environment-specific config file like .mise.<env>.toml", Long: "Create/modify an environment-specific config file like .mise.<env>.toml"},
	...
	{Key: FlagUnusePath, ValueName: "PATH", Short: "Specify a path to a config file or directory", Long: "Specify a path to a config file or directory\n\nIf a directory is specified, it will look for a config file in that directory following the rules above."},
	...
	{Key: FlagUpgradeJobs, ValueName: "JOBS", Short: "Number of jobs to run in parallel", Long: "Number of jobs to run in parallel\n[default: 4]"},
	...
	{Key: FlagUpgradeExclude, Repeatable: true, ValueName: "INSTALLED_TOOL", Short: "Tool(s) to exclude from upgrading", Long: "Tool(s) to exclude from upgrading\ne.g.: go python"},
	...
	{Key: FlagUpgradeMinimumReleaseAge, ValueName: "MINIMUM_RELEASE_AGE", Short: "Only upgrade to versions released before this date or older than this duration", Long: "Only upgrade to versions released before this date or older than this duration\n\nSupports absolute dates like \"2024-06-01\" and relative durations like \"90d\" or \"1y\".\nThis can be useful for reproducibility or security purposes.\n\nThis only affects fuzzy version matches like \"20\" or \"latest\".\nExplicitly pinned versions like \"22.5.0\" are not filtered."},
	...
	{Key: ArgUpgradeInstalledToolVersion, Short: "Tool(s) to upgrade", Long: "Tool(s) to upgrade\ne.g.: node@20 python@3.10\nIf not specified, all current tools will be upgraded"},
	...
	{Key: FlagUseEnv, ValueName: "ENV", Short: "Create/modify an environment-specific config file like .mise.<env>.toml", Long: "Create/modify an environment-specific config file like .mise.<env>.toml"},
	...
	{Key: FlagUseJobs, ValueName: "JOBS", Short: "Number of jobs to run in parallel", Long: "Number of jobs to run in parallel\n[default: 4]"},
	...
	{Key: FlagUsePath, ValueName: "PATH", Short: "Specify a path to a config file or directory", Long: "Specify a path to a config file or directory\n\nIf a directory is specified, it will look for a config file in that directory following the rules above."},
	...
	{Key: FlagUseMinimumReleaseAge, ValueName: "MINIMUM_RELEASE_AGE", Short: "Only install versions released before this date or older than this duration", Long: "Only install versions released before this date or older than this duration\n\nSupports absolute dates like \"2024-06-01\" and relative durations like \"90d\" or \"1y\"."},
	{Key: FlagUsePin, Short: "Save exact version to config file", Long: "Save exact version to config file\ne.g.: `mise use --pin node@20` will save 20.0.0 as the version\nSet `MISE_PIN=1` to make this the default behavior\n\nConsider using mise.lock as a better alternative to pinning in mise.toml:\nhttps://mise.jdx.dev/configuration/settings.html#lockfile"},
	...
	{Key: FlagUseRemove, Repeatable: true, ValueName: "TOOL", Short: "Remove the tool(s) from config file", Long: "Remove the tool(s) from config file"},
	...
	{Key: FlagWatchTaskFlag, Hide: true, Repeatable: true, ValueName: "TASK_FLAG", Short: "Tasks to run", Long: "Tasks to run"},
	{Key: FlagWatchGlob, Hide: true, Repeatable: true, ValueName: "GLOB", Short: "Files to watch", Long: "Files to watch\nDefaults to sources from the task(s)"},
	...
	{Key: FlagWatchColor, ValueName: "MODE", Short: "When to use terminal colours", Long: "When to use terminal colours\n\nSetting the environment variable `NO_COLOR` to any value is equivalent to `--color=never`."},
	...
	{Key: ArgWatchTask, Short: "Tasks to run", Long: "Tasks to run\nCan specify multiple tasks by separating with `:::`\ne.g.: `mise run task1 arg1 arg2 ::: task2 arg1 arg2`"},
	...
	{Key: ArgWhereToolVersion, Demanded: true, Short: "Tool(s) to look up", Long: "Tool(s) to look up\ne.g.: ruby@3\nif \"@<PREFIX>\" is specified, it will show the latest installed version\nthat matches the prefix\notherwise, it will show the current, active installed version"},
	{Key: ArgWhereAsdfVersion, Hide: true, Short: "the version prefix to use when querying the latest version", Long: "the version prefix to use when querying the latest version\nsame as the first argument after the \"@\"\nused for asdf compatibility"},
	...
	{Key: FlagWhichTool, ValueName: "TOOL@VERSION", Short: "Use a specific tool@version", Long: "Use a specific tool@version\ne.g.: `mise which npm --tool=node@20`"},
