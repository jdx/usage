        ...
        const _: () = {
            ...
            impl #ident {
                ...
                pub fn parse_from_argv<'v>(
                    ...
                    {
                        // A view promotes a command surface, not a different package. Its
                        // executable still answers the root's built-in version action before
                        // projection, just as `busybox-applet --version` does.
                        if __usage_words.first().is_some_and(|word| {
                            usage_argv::is_version_arg(SPEC.root.cmd, word)
                        })
                        {
                            return Self::parse_from(__usage_words);
                        }
