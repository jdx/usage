...
impl Cli {
    ...
    pub fn check_position(&self, ident: &syn::Ident, is_root: bool) -> syn::Result<()> {
        if !is_root {
            ...
            // A binding is generated by the root's derive, from the root's fields, so one written
            // here would compile and then contribute nothing: the flag would parse, the setting
            // would keep whatever a file said, and `drift` would report the spec's `cli` node as
            // declared-and-bound-by-nothing. Silence is the failure this attribute exists to
            // prevent, so it is refused rather than accepted and ignored.
            if let Some(field) = self.fields.iter().find(|f| f.setting.is_some()) {
                return Err(syn::Error::new_spanned(
                    &field.ident,
                    "`setting` belongs on a field of the struct that derives `Cli`: only the \
                     root generates the settings layer, so a binding declared here would parse \
                     the flag and set nothing",
                ));
            }
...
mod tests {
    ...
    #[test]
    fn a_setting_outside_the_root_is_refused() {
        // The root's derive generates the layer and the binding table from the root's own fields.
        // A `setting` on a flattened group or a subcommand's struct compiled and then did nothing:
        // the flag parsed, the setting kept whatever a file said, and the only sign was `drift`
        // reporting the spec's `cli` node as bound by nothing. That silence is the failure this
        ...
        assert!(
            err.contains("`setting` belongs on a field of the struct that derives `Cli`"),
            "unhelpful message: {err}"
        );

        // And on the root, which is where it works, it is not an error.
        let parsed = cli(r#"
            struct Ex {
                #[usage(long, setting = "jobs")]
                jobs: Option<usize>,
        ...
        .expect("parses");
        let ident = syn::Ident::new("Probe", proc_macro2::Span::call_site());
        parsed.check_position(&ident, true).expect("allowed here");
    }
