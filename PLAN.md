...
- [x] **`value_delimiter`** — `--tags a,b,c` as three values. `lib/src/spec/arg.rs`
      ...
      checking or converting them. **The fleet fixtures do not yet carry it**: they
      were generated by a clap_usage that recorded a split default and dropped
      the delimiter. Regenerating them against this crate is the item under
      **Trying the fleet**, not another parser feature.
...
- [ ] **The clap-only behaviour the fleet actually uses**, from the list above,
      ...
      | --- | --- | --- |
      | `default_missing_value` / optional-value flags | mise `watch`, hk `-W`, aube `--color`/`--inspect` | `--color` and `--color=always` stop being two spellings of one flag |
      ...
      `value_delimiter` and `requires` are not in that table because the
      _parser_ can say them. They are lost only on the clap → spec round trip,
      and a rewrite that declares in usage keeps them. `allow_hyphen_values` on
      trailing argv is already `double_dash=automatic` in every fleet spec that
      has one.
