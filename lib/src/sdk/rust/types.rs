...
fn render_config_struct(
    ...
    for (field_name, prop) in props {
        ...
        if let Some(default) = &prop.default {
            w.line(&format!(
                "/// Default: {}",
                format_config_default(default, prop.data_type)
            ));
        }
        w.line(&field);
    }
...
fn format_config_default(default: &str, data_type: SpecDataTypes) -> String {
    match data_type {
        SpecDataTypes::Boolean => match default {
            "#true" | "true" => "true".to_string(),
            "#false" | "false" => "false".to_string(),
            other => other.to_string(),
        },
        SpecDataTypes::Integer | SpecDataTypes::Float => default.trim_matches('"').to_string(),
        _ => default.to_string(),
    }
}
