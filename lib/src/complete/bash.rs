...
pub fn complete_bash(opts: &CompleteOptions) -> String {
    ...
    // The completion body below calls `_comp_initialize`, `_comp_compgen` and
    // `_comp_ltrim_colon_completions`, so bash-completion 2.12+ has to be loaded
    ...
    out.push(
        "# Requires bash-completion 2.12 or newer (https://github.com/scop/bash-completion),\n\
         # which must be sourced before this script."
            .to_string(),
    );
    out.push(format!(
        r#"_{bin_snake}() {{
    ...
    if ! declare -F _comp_initialize > /dev/null 2>&1; then
        echo >&2
        echo "Error: bash-completion 2.12+ is required for {bin} completions but was not loaded." >&2
