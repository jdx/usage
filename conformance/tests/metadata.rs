...
/// A CLI whose root says something above and below its help page.
#[derive(Args)]
struct Inner {
    /// A value
...
fn the_roots_surrounding_text_stays_on_the_root_page() {
    ...
    assert!(!page.contains("Read this first."), "{page}");
    assert!(!page.contains("And this after."), "{page}");

    let root_page = usage_argv::help::short_help(spec, &["surrounded"], &[spec.root]);
    assert!(root_page.starts_with("Read this first.\n"), "{root_page}");
    assert!(
        root_page.trim_end().ends_with("And this after."),
        "{root_page}"
    );
    ...
    // The reference agrees after reading the KDL this CLI writes.
    let kdl = spec.to_kdl();
